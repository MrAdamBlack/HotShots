...
func _foobar(unit: Unit) -> void:

	print("Unit_mover.gd: _foobar()")
	var units := get_tree().get_nodes_in_group("units")
	print("unit_mover.gd: get_tree(): " + str(get_tree()))
	print("unit_mover.gd: var Units: " + str(units))
	setup_unit(unit)
	#for unit: Unit in units:
		#print("unit_mover.gd. Unit: ", unit.name, " | Parent: ", get_parent())
		#setup_unit(unit)

func setup_unit(unit: Unit) -> void:
	print("unit_mover.gd: func setup_unit(unit: Unit) -> void:")
	print("unit_mover.gd: setup_unit(): unit is inside tree?", 
			unit.is_inside_tree())
	print("unit_mover.gd: setup_unit(): unit.parent:", unit.get_parent())
	print("unit_mover.gd: setup_unit(): unit.name:", unit.name)
	print("unit_mover.gd: setup_unit(): unit FSM:", unit.unit_state_machine)
	#shop.unit_bought_relay.connect(_foobar.bind(unit))
	#unit.unit_drag_state.drag_started.connect(_foobar.bind(unit))
	unit.unit_drag_state.drag_started.connect(_on_unit_drag_started.bind(unit))
	unit.unit_drag_state.drag_canceled.connect(_on_unit_drag_canceled.bind(unit))
	unit.unit_drag_state.dropped.connect(_on_unit_dropped.bind(unit))
...
func _move_unit(unit: Unit, play_area: PlayArea, tile: Vector2i) -> void:
	print("unit_mover.gd: _move_unit(unit: Unit, play_area: PlayArea, tile: Vector2i) -> void:")
...
func _on_unit_drag_started(unit: Unit) -> void:
	print("unit_mover.gd: _on_unit_drag_started")
	unit.z_index = 99
...
func _on_unit_drag_canceled(starting_position: Vector2, unit: Unit) -> void:
	print("unit_mover.gd: _on_unit_drag_canceled")
...
func _on_unit_dropped(starting_position: Vector2, unit: Unit) -> void:
	print("unit_mover.gd: _on_unit_dropped(starting_position: Vector2, unit: Unit) -> void:")
	_set_highlighters(false)
	...
	if drop_area_index == -1:
		print("unit_mover.gd: if drop_area_index == -1:")
		_reset_unit_to_starting_position(starting_position, unit)
		return
